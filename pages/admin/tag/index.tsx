...
const columns = [
  {
    ...
    render: (text: string, record: any) => (
      <Link href={`/admin/tag/${record.id}`}>{record.name}</Link>
    ),
  },
  {
    title: '描述',
...
const AdminTags: KovaPage<AdminTagsProps> = (props) => {
  const { data, loading, pagination } = useFetchData([], { fetchData: fetchTags })
  useEffect(() => {
    return () => {
      //
    }
